                        ...
                        <div class="orderMake-container">
                            ...
                            <div class="orderMake">
                                <div class="discount-price">
                                    <span class="price-title">活动价</span><span class="price">￥{{discount_price}}</span>
                                </div>
                                <div class="numBox">
                                    <span>{{num1}}件</span>
                                </div>
...
<script>
    export default {
        ...
        created() {
            let order_id = this.getQueryString("order_id");
        ...
        methods:{
            ...
            commit(){
                let order_id = this.getQueryString("order_id");
                if(this.pay_type == 'wx'){
                    this.pay(order_id,this.pay_type);
                    this.wxopen();

                }else if(this.pay_type == 'transfer'){
                    this.transferopen()
                }else if(this.pay_type == 'alipay'){
                    this.alipay(order_id,this.pay_type);
                }

            }
            ,
            getOrderInfo(order_id){
                const url = '/order/pay-again';
                this.$http.post(url,{
                    order_id:order_id
                }).then((res)=>{
                    if(res.body.success == 1){
                       ...
                       this.wxname2 = res.body.weixin;
                       this.wxname = res.body.weixin;
